"""This is a direct copy of shutil.py from the cpython library, available at
https://github.com/python/cpython/blob/main/Lib/shutil.py. It is included here
exclusively for backwards compatability for python 3.6+. All authorship credit
for this file go to PSF and its developers.
...
def _islink(fn):
    return (
        fn.is_symlink() if isinstance(fn, os.DirEntry) else os.path.islink(fn)
    )
...
def copystat(src, dst, *, follow_symlinks=True):
    ...
    lookup("utime")(
        dst, ns=(st.st_atime_ns, st.st_mtime_ns), follow_symlinks=follow
    )
    ...
    try:
        for info in zip.infolist():
            ...
            if not name.endswith("/"):
                # file
                with zip.open(name, "r") as source, open(
                    targetpath, "wb"
                ) as target:
...
def unpack_archive(filename, extract_dir=None, format=None):
    ...
    if format is not None:
        try:
            ...
            raise ValueError(
                "Unknown unpack format '{0}'".format(format)
            ) from None
